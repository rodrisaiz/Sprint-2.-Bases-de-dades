...
SELECT nombre, apellido1 AS 'primer apellido', apellido2 AS 'segundo apellido'  FROM persona WHERE tipo = 'alumno' AND YEAR(fecha_nacimiento) = 1999;
...
SELECT persona.apellido1 AS 'primer apellido', persona.apellido2 AS 'segundo apellido', persona.nombre AS nombre, departamento.nombre AS 'nombre del departamento' FROM persona JOIN profesor ON persona.id = profesor.id_profesor JOIN departamento ON departamento.id = profesor.id_departamento WHERE tipo = 'profesor' ORDER BY persona.apellido1 ASC, persona.apellido2 ASC, persona.nombre ASC ;
...
SELECT departamento.nombre AS 'nombre del departamento', persona.apellido1 AS 'primer apellido', persona.apellido2 AS 'segundo apellido', persona.nombre AS 'nombre' FROM persona LEFT JOIN profesor ON  profesor.id_profesor = persona.id LEFT JOIN departamento ON  profesor.id_departamento = departamento.id WHERE tipo = 'profesor' ORDER BY departamento.nombre ASC, persona.apellido1 ASC, persona.apellido2 ASC, persona.nombre ASC; 
-- 2 - Devuelve un listado con los profesores/as que no están asociados a un departamento.
SELECT  persona.apellido1 AS 'primer apellido', persona.apellido2 AS 'segundo apellido', persona.nombre AS 'nombre' FROM persona LEFT JOIN profesor ON  profesor.id_profesor = persona.id LEFT JOIN departamento ON  profesor.id_departamento = departamento.id WHERE tipo = 'profesor' AND departamento.nombre IS NULL ORDER BY persona.apellido1 ASC, persona.apellido2 ASC, persona.nombre ASC; 
...
SELECT DISTINCT * FROM persona LEFT JOIN profesor ON  profesor.id_profesor = persona.id LEFT JOIN asignatura ON  profesor.id_departamento = asignatura.id_profesor WHERE persona.tipo = 'profesor' AND asignatura.id_profesor IS NULL ORDER BY persona.apellido1 ASC, persona.apellido2 ASC, persona.nombre ASC;
...
SELECT DISTINCT grado.nombre AS 'nombre del grado', asignatura.tipo AS  'tipo_asignatura', (SELECT COUNT(creditos) FROM asignatura WHERE asignatura.tipo = tipo_asignatura) AS 'numero_creditos' FROM asignatura LEFT JOIN grado ON  grado.id = asignatura.id_grado ORDER BY numero_creditos DESC;
...
SELECT  DISTINCT curso_escolar.anyo_inicio  AS 'año', (SELECT COUNT(id_alumno) FROM alumno_se_matricula_asignatura WHERE alumno_se_matricula_asignatura.id_curso_escolar = curso_escolar.id) AS 'numero_alumnos' FROM alumno_se_matricula_asignatura LEFT JOIN asignatura ON  alumno_se_matricula_asignatura.id_asignatura = asignatura.id LEFT JOIN curso_escolar ON  alumno_se_matricula_asignatura.id_curso_escolar = curso_escolar.id ;
...
SELECT * FROM persona LEFT JOIN profesor ON  profesor.id_profesor = persona.id  LEFT JOIN departamento ON  departamento.id = profesor.id_departamento LEFT JOIN asignatura ON profesor.id_profesor = asignatura.id_profesor WHERE persona.tipo = 'profesor'AND asignatura.id IS NULL AND departamento.id IS NOT NULL;
